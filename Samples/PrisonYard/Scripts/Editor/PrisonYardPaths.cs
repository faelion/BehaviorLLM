...
namespace Project.Samples.PrisonYard.Editor
{
    ...
    /// </summary>
    internal static class PrisonYardPaths
    {
        private static string cachedRoot;

        /// <summary>Sample root folder, e.g. <c>Assets/BehaviorLLM/Samples/PrisonYard</c>.</summary>
        ...
        {
            const string fallback = "Assets/BehaviorLLM/Samples/PrisonYard";
            string[] guids = AssetDatabase.FindAssets("t:MonoScript PrisonYardPaths");
            for (int i = 0; i < guids.Length; i++)
            {
                string path = AssetDatabase.GUIDToAssetPath(guids[i]);
                if (Path.GetFileName(path) != "PrisonYardPaths.cs") continue;
            ...
            Debug.LogWarning("[PrisonYard] Could not locate the sample folder from its own scripts; " +
